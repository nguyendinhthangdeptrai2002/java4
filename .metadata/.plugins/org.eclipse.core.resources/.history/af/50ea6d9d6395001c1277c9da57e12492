...
<main class="mt-3 container-fluid">
    <ul class="nav nav-tabs">
        ...
        <li class="nav-item">
            <a class="nav-link" aria-current="page" href="video-edition">Video Edition</a>
        </li>
    </ul>
    <div class="">
        <table class="table table-hover table-striped ">
            ...
            <c:forEach items="${list}" var="user">
                <tr>
                    ...
                    <td>${user.title}</td>
                    <td>${user.views}</td>
                    <td>${user.active == true? "Active": "In-active"}</td>
                    <td><a href="video-edition?id=${user.id}">Edit</a></td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
